...
Physics(MaterialInterface="Insulator/Silicon") {
	Traps(FixedCharge Level Conc= @Bottom_Charge@ )    #0.24e12
	Recombination (SurfaceSRH)
}
Physics(RegionInterface="SiO2_Pocket/FET") {
	Traps(FixedCharge Level Conc= @Side_Charge@ )    #0.3e12
	Recombination (SurfaceSRH)
}
Physics(RegionInterface="MOS_IL/FET") {
	Traps(FixedCharge Level Conc= @Top_Charge@ )    #0.25e12
	Recombination (SurfaceSRH)
}
#}
Plot {
...
#System {
#	MOSFET Structure {"Source"=0 "Drain"=1 "Gate"=2 "Sub"=0}
#	Resistor_pset r1 {resistance = 188k}
#	}
#Solve {
...
#    QuasiStationary ( DoZero InitialStep=0.05 MaxStep = 0.1 MinStep=1e-15 Increment=1.5 Decrement=2
#        Goal { name="Drain"  voltage=@Vd@   }  ) 
#        { coupled { Poisson Electron Hole}  }
#       
 #      
#    NewCurrentPrefix= "idvg@Vd@_"
#    QuasiStationary ( DoZero InitialStep=0.05 MaxStep = 0.03 MinStep=1e-15 Increment=1.5 Decrement=2
#        Goal { name="Gate"  voltage=7   }  ) 
#        { coupled { Poisson Electron Hole}  Plot ( FilePrefix = "TurnOn_n@node@_" Time = ( Range=(0 1) Intervals= 10) NoOverwrite )
#        } 
#       }
Solve {
  Coupled(Iterations=100) { Poisson }
