...
(sdedr:define-constant-profile "PolySi_Definition" "PhosphorusActiveConcentration" 2e20)
...
(sdedr:define-refeval-window "Source_Window" "Rectangle"
   (position 0 (- (/ Wsub 2)) (+ Hsub Hfet))
   (position Lsd (+ (/ Wsub 2)) (+ Hsub Hfet)))

(sdedr:define-refeval-window "Drain_Window" "Rectangle"
   (position Lall (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (- Lall Lsd) (+ (/ Wsub 2)) (+ Hsub Hfet)))

(sdedr:define-refeval-window "Spacer1_Window" "Rectangle"
   (position (+ 0) (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (+ Lsd Lsp) (+ (/ Wsub 2)) (+ Hsub Hfet)))

(sdedr:define-refeval-window "Spacer2_Window" "Rectangle"
   (position (- Lall 0) (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (- Lall (+ Lsd Lsp)) (+ (/ Wsub 2)) (+ Hsub Hfet)))

(sdedr:define-refeval-window "Pocket_Window" "Rectangle"
   (position (- Wsubext) (- 0 Wsubext (/ Wsub 2)) (+ Hsub Hfet))
   (position (+ Lall Wsubext) (+ Wsubext (/ Wsub 2)) (+ Hsub Hfet)))


(define profiles-scm "@ProfilesScm@")

(display ">>> profiles-scm raw value: |")
(display profiles-scm)
(display "|")
(newline)
(display ">>> is string? ")
(display (string? profiles-scm))
(newline)
(display ">>> file-exists? ")
(display (file-exists? profiles-scm))
(newline)

(if (file-exists? profiles-scm)
    (begin
      (display ">>> LOADING profiles from: ")
      (display profiles-scm)
      (newline)
      (load profiles-scm)
      (display ">>> profiles loaded OK")
      (newline))
    (begin
      (display "!!! ERROR: profiles file NOT found: ")
      (display profiles-scm)
      (newline)
      (exit 1)))
...
;--------------------------------------
;Defining Mesh
(sdedr:define-refeval-window "AllMesh_Window" "Cuboid"
   (position (- Wsubext) (- 0 Wsubext (/ Wsub 2)) (- Hsub1))
   (position (+ Lall Wsubext) (+ Wsubext (/ Wsub 2)) (+ Hsub Hfet)))

(sdedr:define-refeval-window "SourceMesh_Window" "Cuboid"
   (position 0 (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (* 1.8 Lsd) (+ (/ Wsub 2)) (- (+ Hsub Hfet) Hsd)))

(sdedr:define-refeval-window "DrainMesh_Window" "Cuboid"
   (position Lall (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (- Lall (* 1.8 Lsd)) (+ (/ Wsub 2)) (- (+ Hsub Hfet) Hsd)))

(sdedr:define-refeval-window "Spacer1Mesh_Window" "Cuboid"
   (position 0 (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (+ Lsd (* 1.8 Lsp)) (+ (/ Wsub 2)) (- (+ Hsub Hfet) (/ Hsd 10))))

(sdedr:define-refeval-window "Spacer2Mesh_Window" "Cuboid"
   (position (- Lall 0) (- (/ Wsub 2)) (+ Hsub Hfet))
   (position (- Lall Lsd (* 1.8 Lsp)) (+ (/ Wsub 2)) (- (+ Hsub Hfet) (/ Hsd 4))))

(sdedr:define-refinement-size "AllMesh_Definition"
   (/ Lc 5) (/ Wsub2 16) (/ Hsub1 10)
   (/ Lc 20) (/ Wsub2 20) (/ Hsub1 30))
(sdedr:define-refinement-function "AllMesh_Definition" "DopingConcentration" "MaxTransDiff" 1)

(sdedr:define-refinement-size "SDMesh_Definition"
   (/ Lsd 3) (/ Wsub 3) (/ Hsd 3)
   (/ Lsd 25) (/ Wsub 25) (/ Hsd 20))
(sdedr:define-refinement-function "SDMesh_Definition" "DopingConcentration" "MaxTransDiff" 1)

(sdedr:define-refinement-size "SpacerMesh_Definition"
   (/ Lsd 3) (/ Wsub 3) (/ Hsd 5)
   (/ Lsd 25) (/ Wsub 15) (/ Hsd 20))
